...
export const getUserById = async (req, res) => {
  try {
    ...
    const user = await User.aggregate([
      ...
      { $lookup: { from: "orders", localField: "_id", foreignField: "customer", as: "userOrders" } },
      { $project: { id: { $toString: "$_id" }, name: { $ifNull: ["$name", "Unnamed Customer"] }, businessName: { $ifNull: ["$businessName", ""] }, businessAddress: { $ifNull: ["$businessAddress", ""] }, contactPerson: { $ifNull: ["$contactPerson", ""] }, panNo: { $ifNull: ["$panNo", ""] }, gstNo: { $ifNull: ["$gstNo", ""] }, fssaiNumber: { $ifNull: ["$fssaiNumber", ""] }, email: { $ifNull: ["$email", ""] }, phone: 1, joinedDate: "$createdAt", status: { $cond: [{ $eq: ["$isActive", false] }, "inactive", "active"] }, totalOrders: { $size: "$userOrders" }, totalSpent: { $sum: "$userOrders.pricing.total" }, lastOrderDate: { $max: "$userOrders.createdAt" }, avatar: customerAvatarProjection, addresses: { $ifNull: ["$addresses", []] }, codBlocked: { $ifNull: ["$codBlocked", false] }, codCancelCount: { $ifNull: ["$codCancelCount", 0] }, codBlockedAt: 1 } },
    ]);
